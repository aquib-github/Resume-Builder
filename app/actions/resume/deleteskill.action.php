...
if (isset($_GET['id']) && isset($_GET['resume_id'])) {
    ...
    if ($id > 0 && $resume_id > 0) {
        ...
        $stmt = $db->prepare("DELETE FROM skills WHERE id = ? AND resume_id = ?");
        $stmt->bind_param("ii", $id, $resume_id);

        try {
            $stmt->execute();
            $stmt->close();
            $fn->setAlert('Skill successfully deleted!');
        } catch (Exception $error) {
            $stmt->close();
            $fn->setError('Failed to delete skill.');
        }
