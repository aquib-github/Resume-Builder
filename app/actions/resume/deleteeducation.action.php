<?php
// Action: Delete an education entry
...
if (isset($_GET['id']) && isset($_GET['resume_id'])) {
    ...
    if ($id > 0 && $resume_id > 0) {
        ...
        $stmt = $db->prepare("DELETE FROM educations WHERE id = ? AND resume_id = ?");
        $stmt->bind_param("ii", $id, $resume_id);

        try {
            $stmt->execute();
            $stmt->close();
            $fn->setAlert('Education successfully deleted!');
        } catch (Exception $error) {
            $stmt->close();
            $fn->setError('Failed to delete education.');
        }
